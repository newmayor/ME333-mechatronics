...
int main(void) {
  ...
  INTCONbits.INT0 = 1;                  // step 3: INT0 and INT1 trigger on rising edge
  INTCONbits.INT1 = 1;
  IPC0bits.CTIP = 0;               // step 4: clear 5 priority and subp bits for INT0
  IPC0bits.CTIP = 24;                 // step 4: set INT0 to priority 6 subpriority 0
  IPC1bits.CTIP = 0;             // step 4: clear 5 priority and subp bits for INT1
  IPC1bits.CTIP = 24;               // step 4: set INT1 to priority 6 subpriority 0
  IFS0bits.INT0IF = 0;              // step 5: clear INT0 flag status
  IFS0bits.INT1IF = 0;              // step 5: clear INT1 flag status
  IEC0bits.CTIE = 1;                   // step 6: enable INT0 and INT1 interrupts
